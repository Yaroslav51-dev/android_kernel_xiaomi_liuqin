...
#define ORDER_ZERO_PAGES (25600)
#define ORDER_FOUR_PAGES (1920)
...
extern int dynamic_page_pool_total(struct dynamic_page_pool *pool, bool high);
...
static struct qcom_system_heap *g_sys_heap;
static char *served_process[POOL_PROCESS_NUMBER] = {"vendor.qti.came", "allocator-servi",
						"composer-servic"};
...
inline bool is_reserve_vmid(int vmid)
{
	if (vmid == RESERVE_VMID)
		return true;
	else
		return false;
}

inline bool can_do_shrink(struct dynamic_page_pool *pool, bool high)
{
	if (pool->vmid == 0 && HIGH_MEM_DEVICE &&
			g_sys_heap->reserve_extend_info.use_reserve_pool) {
		if (pool->order == 0) {
			if (dynamic_page_pool_total(pool, high) > ORDER_ZERO_PAGES)
				return true;
			else
				return false;
		} else if (pool->order == 4) {
			if (dynamic_page_pool_total(pool, high) > ORDER_FOUR_PAGES)
				return true;
			else
				return false;
		} else {
...
{
	return atomic_read(&g_sys_heap->reserve_extend_info
...
static void dynamic_page_pool_refill(struct dynamic_page_pool *pool)
{
	...
	while (g_sys_heap->reserve_extend_info.use_reserve_pool &&
			!dynamic_pool_fillmark_reached(pool) && dynamic_pool_refill_ok(pool)) {
		struct page *page;
		struct reserve_extend *p_re;

		p_re = &g_sys_heap->reserve_extend_info;
		if (!refilled) {
...
bool need_free_to_reserve_pool(struct qcom_system_heap *sys_heap, int order_index)
{
	if (g_sys_heap != NULL && g_sys_heap->reserve_extend_info.use_reserve_pool &&
			g_sys_heap == sys_heap &&
			!dynamic_pool_fillmark_reached(sys_heap->reserve_pool_list[order_index])) {
		if (sys_heap->reserve_extend_info.debug & BACK_TO_RESERVE_POOL)
			pr_info("%s vmid %d order[%d %d]", __func__,
					sys_heap->reserve_pool_list[order_index]->vmid, order_index,
					sys_heap->reserve_pool_list[order_index]->order);
		return true;
	} else
		return false;
...
static void reserve_page_pool_free(struct dynamic_page_pool **reserve_pool)
{
	...
	for (i = 0; i < NUM_ORDERS; i++) {
		...
		if (freed && pool->prerelease_callback)
			ret = pool->prerelease_callback(pool, &pages, freed);
		...
		if (g_sys_heap->reserve_extend_info.debug & CLOSE_RESERVE_POOL)
...
static void reserve_config_init(int (*p_config)[POOL_CONFIG_TYPE_MAX])
{
	...
	g_sys_heap->reserve_extend_info.use_reserve_pool_max_time =
		p_config[g_mem_type_index][POOL_SERVICE_TIME];
	g_sys_heap->reserve_extend_info.start_service_alloc_time =
		p_config[g_mem_type_index][SLOWPATH_TIME];

	for (i = NUM_ORDERS - 1, j = ORDER_ZERO; i >= 0 && j >= 0; i--, j--)
		atomic_set(&g_sys_heap->reserve_extend_info.reserve_order[i],
		...
		struct kobj_attribute *attr, const char *buf, size_t len)
{
	...
	if (value)
		g_sys_heap->reserve_extend_info.use_reserve_pool = true;
	else {
		g_sys_heap->reserve_extend_info.use_reserve_pool = false;
		reserve_page_pool_free(g_sys_heap->reserve_pool_list);
	}
...
static ssize_t debug_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%u\n", g_sys_heap->reserve_extend_info.debug);
}
		...
		struct kobj_attribute *attr, const char *buf, size_t len)
{
	...
	g_sys_heap->reserve_extend_info.debug = value;
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	for (i = 0; i < CONFIG_SHOW_MAX; i++) {
		...
		for (j = 0; j < NUM_ORDERS; j++) {
			...
			if (i == SOURCE_POOL_FREE_ORDER ||
				...
				val = atomic_read(&(config_show_pool[i][j]->count));
			else
	...
	if (len < PAGE_SIZE - 1)
		len += snprintf(buf + len, PAGE_SIZE - len,
				...
				"used_reserve_pool %d\n",
				atomic_read(&g_sys_heap->reserve_extend_info.reserve_slowpath),
				atomic_read(&g_sys_heap->reserve_extend_info.reserve_long_slowpath),
				atomic_read(&g_sys_heap->reserve_extend_info.slowpath),
				g_sys_heap->reserve_extend_info.use_reserve_pool_max_time,
				g_sys_heap->reserve_extend_info.start_service_alloc_time,
				g_sys_heap->reserve_extend_info.use_reserve_pool);
		...
		struct kobj_attribute *attr, const char *buf, size_t len)
{
	...
	g_sys_heap->reserve_extend_info.use_reserve_pool_max_time = val[3];
	g_sys_heap->reserve_extend_info.start_service_alloc_time = val[4];
...
static ssize_t scene_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d\n",g_sys_heap->reserve_extend_info.scene);
}
		...
		struct kobj_attribute *attr, const char *buf, size_t len)
{
	...
	g_sys_heap->reserve_extend_info.scene = scene;
...
static void reserve_extend_info_init(struct qcom_system_heap *sys_heap)
{
	sys_heap->reserve_extend_info.debug = false;

	reserve_config_init(g_reserve_config);

	config_show_pool[SOURCE_POOL_FREE_ORDER] = g_sys_heap->pool_list;
	config_show_pool[RESERVE_POOL_FREE_ORDER] = g_sys_heap->reserve_pool_list;
	config_show_val[RESERVE_POOL_USED_ORDER] = g_sys_heap->reserve_extend_info.use_order;
	config_show_val[RESERVE_POOL_RESERVE_ORDER] = g_sys_heap->reserve_extend_info.reserve_order;
	config_show_val[RESERVE_POOL_REFILL_COUNT] = g_sys_heap->reserve_extend_info.refill_count;
	config_show_val[RESERVE_POOL_REFILL_AMOUNT] = g_sys_heap->reserve_extend_info.refill_amount;
	config_show_val[RESERVE_POOL_REFILL_LAST_AMOUNT] = g_sys_heap->reserve_extend_info.refill_amount_last;
}
...
void qcom_sys_heap_reserve_pool_init(const char *name, struct qcom_system_heap *sys_heap)
{
	...
	set_mem_type_index();
	if (!HIGH_MEM_DEVICE || 0 != strncmp(name, "qcom,system", strlen("qcom,system")))
		return;

	g_sys_heap = sys_heap;
	sys_heap->reserve_pool_list = dynamic_page_pool_create_pools(RESERVE_VMID, NULL);
	pr_info("%s %d %s", current->comm, current->pid, name);
	if (IS_ERR(sys_heap->reserve_pool_list)) {
		sys_heap->reserve_pool_list = NULL;
		g_sys_heap = NULL;
	} else {
		reserve_extend_info_init(sys_heap);
		sys_heap->reserve_extend_info.use_reserve_pool = true;
		reserve_pool_sysfs_create();
		if (IS_ENABLED(CONFIG_QCOM_DMABUF_HEAPS_PAGE_POOL_REFILL)) {
			refill_worker = kthread_run(system_heap_refill_worker,
					sys_heap->reserve_pool_list, "reserve-refill");
			if (IS_ERR(refill_worker)) {
				...
				if (ret) {
					...
					for (i = 0; i < NUM_ORDERS; i++)
						sys_heap->reserve_pool_list[i]->refill_worker =
							refill_worker;
				}
			}
		}
...
static inline void get_pool_count(struct dynamic_page_pool *spool, struct dynamic_page_pool *reserve_pool,
		int *spool_high_count, int *spool_low_count,
		int *reserve_pool_high_count, int *reserve_pool_low_count)
{
	unsigned long flags;

	spin_lock_irqsave(&spool->lock, flags);
	*spool_high_count = spool->high_count;
	*spool_low_count = spool->low_count;
	spin_unlock_irqrestore(&spool->lock, flags);

	spin_lock_irqsave(&reserve_pool->lock, flags);
	*reserve_pool_high_count = reserve_pool->high_count;
	*reserve_pool_low_count = reserve_pool->low_count;
...
static inline struct page *get_page_from_reserve_pool(struct qcom_system_heap *sys_heap, bool served_app,
		struct dynamic_page_pool *spool, struct dynamic_page_pool *reserve_pool)
{
	struct page *page = NULL;
	bool high_order = reserve_pool->order > 0;

	if (sys_heap->reserve_pool_list && served_app) {
		u64 start = 0;

		if (!high_order)
			start = atomic64_read(&sys_heap->reserve_extend_info.use_reserve_pool_start);
		if (high_order || start != 0) {
			u64 delta = 0;

			if (!high_order)
				delta = ktime_to_ms(ktime_get()) - start;
			if (delta < sys_heap->reserve_extend_info.use_reserve_pool_max_time) {
				page = get_page_from_pool(reserve_pool);
				if (sys_heap->reserve_extend_info.debug & RESERVE_POOL)
					pr_info_normal("reserve pool", page, served_app, spool, reserve_pool);

				if (page != NULL)
					atomic_inc(&sys_heap->reserve_extend_info.use_order[order_to_index(reserve_pool->order)]);
			} else {
				atomic64_set(&sys_heap->reserve_extend_info.use_reserve_pool_start, 0);
				if (sys_heap->reserve_extend_info.debug & RESERVE_POOL_START)
					pr_info_normal("used reserve pool timeout", page, served_app, spool, reserve_pool);
...
static inline struct page *get_high_order_page_from_buddy(struct qcom_system_heap *sys_heap, bool served_app,
		struct dynamic_page_pool *spool, struct dynamic_page_pool *reserve_pool)
{
	struct page *page = NULL;
	...
	if (spool->order > 0) {
		...
		if (sys_heap->reserve_extend_info.debug & HIGH_ORDER_SYSTEM_ALLOC)
...
static inline struct page *get_page_from_buddy(struct qcom_system_heap *sys_heap, bool served_app,
		struct dynamic_page_pool *spool, struct dynamic_page_pool *reserve_pool)
{
	s64 delta;
	...
	if (served_app &&
			delta > sys_heap->reserve_extend_info.start_service_alloc_time) {
		atomic64_set(&sys_heap->reserve_extend_info.use_reserve_pool_start, ktime_to_ms(ktime_get()));
		atomic_inc(&sys_heap->reserve_extend_info.reserve_long_slowpath);
		if (sys_heap->reserve_extend_info.debug & RESERVE_POOL_START)
			pr_info_with_alloc_info("reserve-sp", page, served_app,
					spool, reserve_pool, delta, &spool->gfp_mask);
	}

	if (sys_heap->reserve_extend_info.debug & ALL_ORDER_SYSTEM_ALLOC) {
	...
	if (spool->order == 0) {
		if (served_app)
			atomic_inc(&sys_heap->reserve_extend_info.reserve_slowpath);
		else
			atomic_inc(&sys_heap->reserve_extend_info.slowpath);
	}
...
struct page *qcom_extend_sys_heap_alloc_largest_available(struct qcom_system_heap *sys_heap,
						   unsigned long size,
						   unsigned int max_order)
{
	struct page *page = NULL;
	struct dynamic_page_pool **pools = sys_heap->pool_list;
	struct dynamic_page_pool **reserve_pools = sys_heap->reserve_pool_list;
	...
	for (i = 0; i < NUM_ORDERS; i++) {
		...
		page = get_page_from_pool(pools[i]);
		if (!page)
			page = get_high_order_page_from_buddy(sys_heap, served_app,
					pools[i], reserve_pools[i]);

		if (!page)
			page = get_page_from_reserve_pool(sys_heap, served_app,
					pools[i], reserve_pools[i]);

		if (sys_heap->reserve_extend_info.debug & ALL_CALLER)
			pr_info_normal("All", page, served_app,
					pools[i], reserve_pools[i]);

		if (!page) {
			page = get_page_from_buddy(sys_heap, served_app,
					pools[i], reserve_pools[i]);
		}
		...
		if (!page)
		 ...
		 wakeup_refill_thread_by_wmark(served_app, pools[i], reserve_pools[i]);
